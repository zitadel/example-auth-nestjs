...
export function getMessage(
  errorInput: string | ParsedQs | Array<string | ParsedQs> | null | undefined,
  category: ErrorCategory,
): { heading: string; message: string } {
  const errorCode: string | null = Array.isArray(errorInput)
    ? errorInput[0] == null
      ? null
      : typeof errorInput[0] === 'string'
        ? errorInput[0]
        : errorInput[0].toString()
    : errorInput == null
      ? null
  ...
  const normalized = errorCode?.toLowerCase() ?? 'default';
